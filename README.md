...
1. micro:bit 側（最初の1回だけ）
   ...
   - `microbit-bridge.js`（標準版・**拡張機能不要**）を丸ごと貼り付けて「ダウンロード」
   - LEDテープ(NeoPixel)を使う人だけ `microbit-bridge-neopixel.js` を使う。
     こちらは**貼り付ける前に**「拡張機能」から `neopixel` の追加が必要
     （「名前空間 'neopixel' が見つかりません」エラー＝拡張機能が未追加）。
     標準版は C/N/F/W（LEDテープ命令）を無視する
2. PC 側（毎回）
